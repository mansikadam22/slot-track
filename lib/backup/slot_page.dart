...
class _SlotpageState extends State<Slotpage> {
  ...
  void initState() {
    super.initState();
    fetchSlotData(); // initial fetch
    _timer = Timer.periodic(const Duration(seconds: 3), (_) => fetchSlotData());
  }
  ...
  Future<void> fetchSlotData() async {
    try {
      final response = await http.get(
        Uri.parse('https://devmindssmartsmsappapi.devminds.co.in/slotLiveStatus'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          slots = {
            for (var slot in slotList) slot['slotName']: !slot['occupied']
          };
        });
      } else {
        print("Failed to fetch slot data: ${response.statusCode}");
      }
    } catch (e) {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            _statusBox(availableSlots, totalSlots),
  ...
  Widget _slotBox(String slot) {
    bool isAvailable = slots[slot] ?? false;
        ...
        child: Row(
          children: [
            Icon(Icons.directions_car_filled,
                size: 28, color: isAvailable ? Colors.green : Colors.red),
